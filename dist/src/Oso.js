...
class Oso extends Polar_1.Polar {
    ...
    async authorizedQuery(actor, action, resourceCls) {
        ...
        const results = this.queryRuleWithBindings('allow', bindings, actor, action, resource);
        const queryResults = [];
        for await (const result of results) {
            queryResults.push({
        ...
        const plan = this.getFfi().buildFilterPlan(host.serializeTypes(), queryResults, 'resource', clsName);
        return dataFiltering_1.filterData(host, plan);
    }
    /**
    ...
    async authorizedResources(actor, action, resourceCls) {
        ...
        if (userType === undefined)
            throw new errors_1.UnregisteredClassError(resourceCls.name);
        return (await userType.execQuery(query));
    }
    /**
    ...
    setDataFilteringQueryDefaults(options) {
        if (options.buildQuery)
            this.getHost().buildQuery = options.buildQuery;
        if (options.execQuery)
            this.getHost().execQuery = options.execQuery;
